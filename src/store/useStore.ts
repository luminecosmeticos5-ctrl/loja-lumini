...
export const useStore = create((set: any, get: any) => ({
  ...
  fetchFromSupabase: async (force = false) => {
    // Basic caching
    if (!force && get().products.length > 0 && get().lastFetch && (Date.now() - get().lastFetch < 300000)) return;
    ...
    try {
      ...
      if (dbProducts) {
        const mappedProducts = dbProducts.map(p => ({
          ...
          price_original: p.preco_original,
          price_current: p.preco_atual,
          discount: Math.round(((p.preco_original - p.preco_atual) / (p.preco_original || 1)) * 100),
          images: p.imagens || [],
      ...
      if (!cError && dbCategories) set({ categories: dbCategories });
      if (!bError && dbBanners) set({ banners: dbBanners });
      if (!eError && dbEditorial) set({ editorial: dbEditorial });
      if (!prError && dbPromotional) set({ promotionalBanners: dbPromotional });
      if (!pgError && dbPages) set({ pages: dbPages });
      if (!aError && dbAppearance) {
        set({ appearance: dbAppearance });
        // Apply appearance settings only to .store-theme
        ...
        if ((dbAppearance as any).layout) {
          ...
          if (layout.card_radius !== undefined) document.documentElement.style.setProperty('--store-radius-card', `${layout.card_radius}px`);
          if (layout.button_radius !== undefined) document.documentElement.style.setProperty('--store-radius-button', `${layout.button_radius}px`);
          
          // Header specific tokens from layout config
          if (layout.header_font) document.documentElement.style.setProperty('--store-header-font', layout.header_font);
          if (layout.header_weight) {
             document.documentElement.style.setProperty('--store-header-weight', layout.header_weight);
             document.documentElement.style.setProperty('--store-font-weight-menu', layout.header_weight);
          }
          if (layout.header_spacing) document.documentElement.style.setProperty('--store-header-spacing', `${layout.header_spacing}em`);
          if (layout.header_transform) document.documentElement.style.setProperty('--store-header-transform', layout.header_transform);
          if (layout.header_size) document.documentElement.style.setProperty('--store-header-size', `${layout.header_size}px`);

          // Load header font if it is different
          if (layout.header_font) {
             ...
             link.href = `https://fonts.googleapis.com/css2?family=${layout.header_font.replace(/\s+/g, '+')}:wght@100;200;300;400;500;600;700;800;900&display=swap`;
          }
        }
        
        if (dbAppearance.fontes) {
          const fonts = dbAppearance.fontes as any;
          if (fonts.heading) {
            document.documentElement.style.setProperty('--store-font-primary', fonts.heading);
            document.documentElement.style.setProperty('--store-font-heading', fonts.heading);
          }
          if (fonts.body) document.documentElement.style.setProperty('--store-font-body', fonts.body);
          ...
          if (fontsToLoad.length > 0) {
            ...
            const fontFamilies = uniqueFonts.map(f => `${f.replace(/\s+/g, '+')}:wght@100;200;300;400;500;600;700;800;900`).join('&family=');
            link.href = `https://fonts.googleapis.com/css2?family=${fontFamilies}&display=swap`;
          }
        }

        if (dbAppearance.favicon_url) {
          ...
          favicon.setAttribute('href', dbAppearance.favicon_url);
        }
      }
      if (!sError && dbSections) set({ homeSections: dbSections });
      if (!cfgError && dbConfig) {
        const formattedConfig = dbConfig.reduce((acc: any, item: any) => {
          acc[item.chave] = item.valor;
          return acc;
        }, {});
        set((state: any) => ({ config: { ...state.config, ...formattedConfig } }));
      }
  ...
  addToCart: (product: any, color: any, size: any) => {
    const productId = typeof product.id === 'object' ? product.id.id || JSON.stringify(product.id) : product.id;
    const existing = get().cart.find((item: any) => item.id === productId && item.color === color && item.size === size);
    ...
    if (existing) {
      ...
      newCart = [...get().cart, { ...product, id: productId, color, size, quantity: 1 }];
    }
  ...
  removeFromCart: (id: any, color: any, size: any) => {
    const newCart = get().cart.filter((item: any) => !(item.id === id && item.color === color && item.size === size));
    set({ cart: newCart });
    localStorage.setItem('loja-cart', JSON.stringify(newCart));
  },
  ...
  updateConfig: (newConfig: any) => set((state: any) => ({
