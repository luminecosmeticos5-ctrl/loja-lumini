...
const Header = ({ onMenuClick }: { onMenuClick: () => void }) => {
  ...
  const handleSearch = (e: React.FormEvent) => {
    e.preventDefault();
    if (searchTerm.trim()) {
      // Small delay for the ripple/premium effect
      setTimeout(() => {
  ...
  return (
    <header className="sticky top-0 z-50 w-full bg-[var(--header-background)] shadow-lg border-b border-brand-border/10 min-h-[60px]">
      <div className="max-w-[1400px] mx-auto px-4 lg:px-8 py-3 lg:py-5 flex items-center justify-between gap-4">
        {/* Mobile Menu Toggle */}
        <button 
          onClick={onMenuClick}
          className="lg:hidden text-[var(--header-foreground)] p-2 -ml-2 hover:bg-white/10 rounded-full transition-colors"
        ...
        <Link 
          to="/" 
          className="text-[var(--header-foreground)] flex flex-col items-start group transition-all duration-300 shrink-0"
          style={{ 
        ...
        {/* Action Icons */}
        <div className="flex items-center gap-2 md:gap-4 lg:gap-8 text-[var(--header-foreground)] shrink-0">
            <Link to={user ? "/minha-conta" : "/login"} className="flex flex-col items-center gap-1 group transition-all duration-200">
              ...
              <span className="text-[8px] lg:text-[10px] text-[var(--header-foreground)] opacity-90 hidden sm:block"
                style={{ 
                  fontFamily: 'inherit',
                  fontWeight: 'var(--store-font-weight-menu)',
                  letterSpacing: 'var(--store-header-spacing)',
            ...
            <Link to="/favoritos" className="flex flex-col items-center gap-1 group transition-all duration-200 hidden sm:flex">
              ...
              <span className="text-[8px] lg:text-[10px] text-[var(--header-foreground)] opacity-90"
                style={{ 
                  fontFamily: 'inherit',
                  fontWeight: 'var(--store-font-weight-menu)',
                  letterSpacing: 'var(--store-header-spacing)',
            ...
            >
              <div className="relative">
                ...
                {cartCount > 0 && (
                  <span className="absolute -top-2 -right-2.5 bg-brand-primary text-brand-primary-foreground text-[9px] lg:text-[10px] font-black h-4 w-4 lg:h-5 lg:w-5 rounded-full flex items-center justify-center shadow-lg border-2 border-[var(--header-background)]">
                    {cartCount}
                  </span>
                )}
              </div>
      ...
      {/* Mobile Search */}
      <div className="px-4 pb-4 lg:hidden bg-[var(--header-background)]">
        <form onSubmit={handleSearch} className="search-wrapper">
          <input 
            type="text" 
